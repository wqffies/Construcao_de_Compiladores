...
def gerarCodigo(arvore):
    if isinstance(arvore, Const):
        # Caso base - garantir indentação de 2 espaços
        return f"  mov ${arvore.valor}, %rax"
    elif isinstance(arvore, OpBin):
        # Processar operando ESQUERDO primeiro
        ...
        # Combina os códigos com indentação correta
        codigo = (
            f"{codigo_esq}\n"    # Operando esquerdo em RAX
            f"  push %rax\n"     # Salva o resultado do operando esquerdo na pilha
            f"{codigo_dir}\n"    # Operando direito em RAX  
            f"  pop %rbx"        # Recupera o resultado do operando esquerdo da pilha
        )
        
        # Adiciona a operação correspondente
        if arvore.operador == '+':
            codigo += "\n  add %rbx, %rax"  # rax = rax + rbx (direito + esquerdo)
        elif arvore.operador == '-':
            # Para subtração, precisa ser RBX - RAX (esquerdo - direito)
            codigo += "\n  sub %rax, %rbx"  # rbx = rbx - rax (esquerdo - direito)
            codigo += "\n  mov %rbx, %rax"  # Move resultado para RAX
        elif arvore.operador == '*':
            codigo += "\n  imul %rbx, %rax"  # rax = rax * rbx (direito * esquerdo)
        elif arvore.operador == '/':
            # Para divisão, precisa ser RBX / RAX (esquerdo / direito)
            codigo += (
                "\n  xchg %rax, %rbx"  # Troca RAX e RBX (agora RAX = esquerdo, RBX = direito)
                "\n  cqo"              # Estende o sinal de RAX para RDX:RAX
                "\n  idiv %rbx"        # Divide RDX:RAX por RBX, resultado em RAX
            )
        else:
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Uso: python atividade6.py arquivo_entrada.ec")
        sys.exit(1)
